...
| 05 | AI systems | [ai/coding-agent.html](ai/coding-agent.html) | A coding agent on a seven-file repo with a real test harness: reproduce, read what the failure points at, one edit, verify, stop; then the blind, greedy and no-verify failure modes, and a subagent returning a summary |
| 06 | AI systems | [ai/progressive-disclosure.html](ai/progressive-disclosure.html) | Context engineering: core rules, a one-line skill index, bodies fetched on demand; eager loading's cost and confusion as a catalogue grows to fifty, the cost/accuracy curve, vague descriptions breaking the index, and a window too small for eager loading |
| 07 | AI systems | [ai/tokens-sampling.html](ai/tokens-sampling.html) | Real BPE trained on a small corpus with a merge slider, rare words shattering, letter counting across pieces; a trigram model's next-token distribution reshaped by temperature, top-k and top-p, greedy loops, seeded sampling |
